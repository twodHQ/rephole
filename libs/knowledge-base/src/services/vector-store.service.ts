...
import {ChromaClient, Collection, Metadata, QueryResult, Where} from 'chromadb';
import {CHROMA_CLIENT} from '@app/core';
import type {ChromaMetadata, EmbeddingVector, VectorMetadata, VectorRecord,} from '@app/shared-interfaces';
...
export class VectorStoreService {
    ...
    async similaritySearch(
        ...
        this.logger.debug(
            `[similaritySearch] Searching with filter: ${JSON.stringify(filter)}, topK: ${topK}`,
        );

        // First, check total count in collection
        const totalCount = await collection.count();
        this.logger.debug(
            `[similaritySearch] Total documents in collection: ${totalCount}`,
        );
        ...
        this.logger.debug(`[similaritySearch] Found ${resultCount} results`);
